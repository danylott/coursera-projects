...
def apply_god_light(engine, hero):
    engine.score += 0.3
    engine.hero = Objects.GodLight(hero)
    engine.notify("God Light applied")
            ...
            for obj_name in object_list_prob['enemies']:
                ...
                for i in range(random.randint(0, 5)):
                    ...
                    self.objects.append(Objects.Enemy(
                        prop['sprite'], prop, prop['experience'], coord, prop['action']))
...
def service_init(sprite_size, full=True):
    ...
    object_list_actions = {'reload_game': reload_game,
                           ...
                           'apply_god_light': apply_god_light}
    ...
    for enemy in object_list_prob['enemies']:
        ...
        prop['action'] = object_list_actions.get(prop_tmp.get('action', None), None)
    file.close()
