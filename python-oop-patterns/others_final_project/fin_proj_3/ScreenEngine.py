...
class GameSurface(ScreenHandle):
    ...
    def draw_hero(self):
        self.engine.hero.draw(self, self.engine.sprite_size)

    def draw_map(self):

        # FIXME || calculate (min_x,min_y) - left top corner

        min_x = - self.engine.start_position[0] + self.engine.hero.position[0]
        min_y = - self.engine.start_position[1] + self.engine.hero.position[1]
        ...
        if self.engine.map:
            for i in range(len(self.engine.map[0]) - min_x):
                for j in range(len(self.engine.map) - min_y):
                    self.blit(self.engine.map[min_y + j][min_x + i][0], (i * self.engine.sprite_size,\
...
class GameMiniMap(ScreenHandle):
    ...
    def draw_hero(self):
        #self.engine.hero.draw(self, 10)
        min_x =   self.engine.start_position[0]
        min_y =   self.engine.start_position[1]
        pygame.draw.rect(self, colors["red"], (min_x+20,
                                               min_y + 20, min_x +8, min_y + 8))
    ...
    ##
        ...
        if self.engine.map:
            for i in range(len(self.engine.map[0]) - min_x):
                for j in range(len(self.engine.map) - min_y):
                    self.blit(self.engine.map[min_y + j][min_x + i][0], (i * 8,\
                                 j * 8))
        else:
            self.fill(colors["white"])
    ...
    # FIXME || calculate (min_x,min_y) - left top corner
        ...
        pygame.draw.rect(self, colors["red"],((coord[0] - min_x) * 8,(coord[1] - min_y) * 8,
                                              (coord[0] - min_x) * 8,(coord[1] - min_y) * 8))
        #self.blit(sprite, ((coord[0] - min_x) * 8,
                           #(coord[1] - min_y) * 8))
    ...
    ##
        self.draw_map()
        for obj in self.engine.objects:
            pygame.draw.circle(self, colors["green"], ((obj.position[0] - min_x) * 10 , (obj.position[1] - min_y) * 10), 5)

            #self.blit(obj.sprite[0], ((obj.position[0] - min_x) * 8,
                                     # (obj.position[1] - min_y) * 8))
        self.draw_hero()
